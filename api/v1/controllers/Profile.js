...
Controller.prototype.updateAccount = (req, res) => {
	if (!req.auth.user_id) {
		response.sendFail(res, "Unauthorised access!");
	} else {
		Users.update(req.auth.user_id, { name: validator.escape(req.body.name) }).then(function (updated) {
			if (updated) {
				response.sendSuccess(res, "Profile Updated");
			} else {
				response.sendFail(res, "Profile update failed");
			}
		}).catch(function (err) {
			response.sendFail(res, "Profile update failed");
		});
