...
Controller.prototype.send_otp = (req, res) => {
	if (!req.body.user_id || !req.body.phone_no || !req.body.ip || !req.body.user_agent || !req.body.count_code) {
		response.sendFail(res, "Phone number is required");
	} else {
		var user_id = req.body.user_id;
		var phone_no = Number(req.body.phone_no);

		if (validator.isUUID(user_id)) {
			if (phone_no <= 10 && count_code != " ") {
				Users.generateOtp(length == 6).then(function (data) {
					var otp = data;

				   Users.sendtoOtp(phone_no, otp).then(function (res_data) {
                    if(res_data.status == true){
						response.sendSuccess(res, "OTP send successfully");
					}else{
						response.sendFail(res, "OTP Failed");
					}

				    }).catch(function (err) {
					response.sendFail(res, "OTP Failed. Try Again.");
				    });
				}).catch(function (err) {
					response.sendFail(res, "Something wrong. Please contact to admin.");
				});
			} else {
				response.sendFail(res, "Invalid phone number");
			}
		} else {
			response.sendFail(res, "Invalid User id");
		}
